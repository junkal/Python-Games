...
class Game:
    def __init__(self):
        ...
        # Entities
        self.background_img = self.asset_loader.load_image("background", "background.png", scale=(settings.SCREEN_WIDTH, settings.SCREEN_HEIGHT))
    ...
    def update(self):
        ...
        for bullet in self.bullet_group:
            ...
            if hit_enemies:
                for enemy in hit_enemies:
                    if enemy.hit():  # ✅ reduces HP, kills only if hp <= 0
        ...
        if not self.enemy_group:
            ...
            # Next level
            self.level_manager.next_level(self.enemy_group, self.enemy_images)
    ...
    def spawn_explosion(self, x, y):
        explosion = Explosion(x, y, self.explosion_img)
